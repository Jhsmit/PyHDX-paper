...
fit_dir = 'fits'
reg_dir = f"r1_{reg}"
files = [current_dir / fit_dir / state / reg_dir / 'fit_result.csv' for state in f_states]
...
files = [current_dir / fit_dir / state / reg_dir / 'fit_result.csv' for state in g_states]
deltaGs = np.array([csv_to_protein(f)['deltaG'].dropna().to_numpy()*1e-3 for f in files], dtype=object)
...
fig, axes = pplt.subplots(ncols=2, width=183.0/25.4, aspect=5, ref=1, wratios=[4, 1], sharey=False)
ax = axes[0]
