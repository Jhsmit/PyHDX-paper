...
aspect = 15
height = 35
fig, ax = pplt.subplots(height=height/25.4, aspect=1/15)

ticks = np.linspace(10, 40, endpoint=True, num=4).astype(int)[::-1]
tick_pos = rgb_norm(ticks)
cbar = ax.colorbar(rgb_cmap.reversed(), loc='fill', ticks=tick_pos, orientation='vertical', width=cbar_width)
cbar.ax.yaxis.tick_right()
cbar.set_label(ddG_ylabel, labelpad=0)
cbar.ax.set_yticklabels(ticks)
cbar.ax.yaxis.set_label_position('right')
ax.format(ylabel=dG_ylabel)

#plt.show()
plt.savefig(output_dir / 'Colorbar_abs_gibbs_vertical.png')
plt.savefig(output_dir / 'Colorbar_abs_gibbs_vertical.pdf')

fig, ax = pplt.subplots(height=height/25.4, aspect=1/15)
ticks = np.linspace(-10, 10, endpoint=True, num=5).astype(int)[::-1]

tick_pos = diff_norm(ticks)
cbar = ax.colorbar(diff_cmap.reversed(), loc='fill', ticks=tick_pos, orientation='vertical', width=cbar_width)
cbar.ax.yaxis.tick_right()
cbar.set_label(ddG_ylabel, labelpad=0)
cbar.ax.set_yticklabels(ticks)
cbar.ax.yaxis.set_label_position('right')
ax.format(ylabel=ddG_ylabel)

#plt.show()
plt.savefig(output_dir / 'Colorbar_diff_gibbs_vertical.png')
plt.savefig(output_dir / 'Colorbar_diff_gibbs_vertical.pdf')