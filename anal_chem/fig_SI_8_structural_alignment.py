...
df = csv_to_protein(fitresults_dir / 'batch_fits' / 'ecSecB_mtSecB' / alignment_folder / f'fit_result.csv')
...
for orientation, view in zip(['top', 'side'], aligned_views):
    ...
    cmd.png(str(output_dir / f'{fname}_{orientation}_no_gibbs.png'))
    ...
    cmd.ray(px, py, renderer=0, antialias=2)
    cmd.png(str(output_dir / f'{fname}_{orientation}_gibbs.png'))

    cmd.disable('mtSecB')
    cmd.ray(px, py, renderer=0, antialias=2)
    cmd.png(str(output_dir / f'{fname}_{orientation}_gibbs_ecsecb.png'))

    cmd.disable('ecSecB')
    cmd.enable('mtSecB')
    cmd.ray(px, py, renderer=0, antialias=2)
    cmd.png(str(output_dir / f'{fname}_{orientation}_gibbs_mtsecb.png'))
